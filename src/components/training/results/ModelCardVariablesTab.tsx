...
function findWarning(warnings: ArtifactWarning[] | undefined, artifact: string): ArtifactWarning | undefined {
  return warnings?.find((w) => w.artifact === artifact);
}
...
export function ModelCardVariablesTab({ explainability }: ModelCardVariablesTabProps) {
  const featureChartData = buildFeatureImportanceChartData(explainability);
  const pi = explainability.permutationImportance;
  const sg = explainability.shapGlobal;
  const artifactWarnings = explainability.artifactWarnings;
  ...
  return (
    <div className="space-y-4">
      ...
      {(!pi || pi.length === 0) && (() => {
        const w = findWarning(artifactWarnings, 'permutation_importance');
        return w ? <ArtifactUnavailable label="Importance par permutation" reason={`${w.error} : ${w.detail}`} /> : null;
      })()}
      ...
      {!sg && (() => {
        const w = findWarning(artifactWarnings, 'shap');
...
export function ModelCardCourbesTab({ curves }: ModelCardCourbesTabProps) {
  const lcWarning = findWarning(curves.artifactWarnings, 'learning_curves');
  ...
  return (
    <div className="space-y-4">
            ...
            {curves.roc && (
              <div>
                ...
                <div className="rounded-lg border border-border/50 bg-muted/20 p-1.5" style={{ height: 160 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart
                      data={curves.roc.map(([fpr, tpr]) => ({ fpr, tpr }))}
                    ...
                    >
                      ...
                      <XAxis
                        ...
                        tickCount={5}
                        tick={{ fontSize: 10 }}
                        label={{ value: 'FPR', position: 'insideBottom', offset: -4, fontSize: 10 }}
                      />
                      <YAxis
                        type="number"
                        domain={[0, 1]}
                        tickCount={5}
                        tick={{ fontSize: 10 }}
                        label={{ value: 'TPR', angle: -90, position: 'insideLeft', offset: 8, fontSize: 10 }}
                      />
                      <Tooltip
            ...
            {curves.pr && (
              <div>
                ...
                <div className="rounded-lg border border-border/50 bg-muted/20 p-1.5" style={{ height: 160 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart
                      data={curves.pr.map(([recall, precision]) => ({ recall, precision }))}
                      margin={{ top: 4, right: 8, bottom: 16, left: 8 }}
                    >
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(128,128,128,0.15)" />
                      <XAxis
                      ...
                      <YAxis
                        ...
                        tickCount={5}
                        tick={{ fontSize: 10 }}
                        label={{ value: 'Precision', angle: -90, position: 'insideLeft', offset: 8, fontSize: 10 }}
                      />
                      <Tooltip
                        formatter={(v: number) => v.toFixed(3)}
            ...
            {curves.calibration && (
              <div>
                <div className="mb-1 flex items-center justify-center gap-1.5">
                  ...
                  >
                    Brier {curves.calibration.brier_score.toFixed(3)}
                  </span>
                </div>
                <div className="rounded-lg border border-border/50 bg-muted/20 p-1.5" style={{ height: 160 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart
                      data={curves.calibration.points.map(([mp, fp]) => ({ predicted: mp, actual: fp }))}
                      margin={{ top: 4, right: 8, bottom: 16, left: 8 }}
                    >
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(128,128,128,0.15)" />
                      <XAxis
                        ...
                        tickCount={5}
                        tick={{ fontSize: 10 }}
                        label={{ value: 'Probabilité prédite', position: 'insideBottom', offset: -4, fontSize: 10 }}
                      />
                      <YAxis
                        type="number"
                        domain={[0, 1]}
                        tickCount={5}
                        tick={{ fontSize: 10 }}
                        label={{ value: 'Fraction réelle', angle: -90, position: 'insideLeft', offset: 8, fontSize: 10 }}
                      />
                      <Tooltip
                        formatter={(v: number) => v.toFixed(3)}
      ...
      {!curves.learningCurves && lcWarning && (
        <ArtifactUnavailable label="Courbes d'apprentissage" reason={`${lcWarning.error} : ${lcWarning.detail}`} />
      )}

      {curves.learningCurves && (() => {
        const lc = curves.learningCurves as LearningCurveData;
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={lcData} margin={{ top: 4, right: 8, bottom: 16, left: 8 }}>
                  ...
                  <XAxis
                    dataKey="samples"
                    tick={{ fontSize: 10 }}
                    label={{ value: 'Échantillons', position: 'insideBottom', offset: -4, fontSize: 10 }}
                  />
                  <YAxis
                    domain={['auto', 'auto']}
                    tick={{ fontSize: 10 }}
